// This file is auto-generated by @hey-api/openapi-ts

import { type Options as ClientOptions, type TDataShape, type Client, formDataBodySerializer } from '@hey-api/client-fetch';
import type { GetApiV2AiServiceCompletionsCompleteData, GetApiV2AiServiceCompletionsCompleteError, GetAllCountriesData, GetAllCountriesResponse, GetAllCountriesError, CountCountriesData, CountCountriesResponse, CountCountriesError, SearchCountriesByNameAsyncData, SearchCountriesByNameAsyncResponse, SearchCountriesByNameAsyncError, GetCountryByIdData, GetCountryByIdResponse, GetCountryByIdError, GetCountryStatesAsyncData, GetCountryStatesAsyncResponse, GetCountryStatesAsyncError, CountCountryStatesAsyncData, CountCountryStatesAsyncResponse, CountCountryStatesAsyncError, GetCountryStateByIdAsyncData, GetCountryStateByIdAsyncResponse, GetCountryStateByIdAsyncError, GetCitiesByCountryStateIdAsyncData, GetCitiesByCountryStateIdAsyncResponse, GetCitiesByCountryStateIdAsyncError, CountCitiesByStateAsyncData, CountCitiesByStateAsyncResponse, CountCitiesByStateAsyncError, GetTimeZonesByCountryIdAsyncData, GetTimeZonesByCountryIdAsyncResponse, GetTimeZonesByCountryIdAsyncError, CountTimezonesByCountryAsyncData, CountTimezonesByCountryAsyncResponse, CountTimezonesByCountryAsyncError, GetEnabledCurrenciesByCountryIdAsyncData, GetEnabledCurrenciesByCountryIdAsyncResponse, GetEnabledCurrenciesByCountryIdAsyncError, GetTopLevelDomainsByCountryIdAsyncData, GetTopLevelDomainsByCountryIdAsyncResponse, GetTopLevelDomainsByCountryIdAsyncError, CountTopLevelDomainsByCountryAsyncData, CountTopLevelDomainsByCountryAsyncResponse, CountTopLevelDomainsByCountryAsyncError, GetCallingCodesByCountryIdAsyncData, GetCallingCodesByCountryIdAsyncResponse, GetCallingCodesByCountryIdAsyncError, CountCallingCodesByCountryAsyncData, CountCallingCodesByCountryAsyncResponse, CountCallingCodesByCountryAsyncError, GetEnabledCurrenciesAsyncData, GetEnabledCurrenciesAsyncResponse, GetEnabledCurrenciesAsyncError, CountCurrenciesAsyncData, CountCurrenciesAsyncResponse, CountCurrenciesAsyncError, GetCurrencyByIdAsyncData, GetCurrencyByIdAsyncResponse, GetCurrencyByIdAsyncError, GetVersionData, GetHealthData, GetHelloData, PostRegisterData, PostRegisterError, PostLoginData, PostLoginResponse, PostRefreshData, PostRefreshResponse, MapIdentityApiConfirmEmailData, PostResendConfirmationEmailData, PostForgotPasswordData, PostForgotPasswordError, PostResetPasswordData, PostResetPasswordError, PostManage2FaData, PostManage2FaResponse, PostManage2FaError, GetManageInfoData, GetManageInfoResponse, GetManageInfoError, PostManageInfoData, PostManageInfoResponse, PostManageInfoError, PostAccountPerformExternalLoginData, PostAccountLogoutData, PostAccountManageLinkExternalLoginData, PostAccountManageDownloadPersonalDataData, GetLanguagesAsyncData, GetLanguagesAsyncResponse, GetLanguagesAsyncError, CountLanguagesAsyncData, CountLanguagesAsyncResponse, CountLanguagesAsyncError, GetLanguageByIdAsyncData, GetLanguageByIdAsyncResponse, GetLanguageByIdAsyncError, PostApiV2GlobalSystemMigrateData, PostApiV2GlobalSystemMigrateResponse, PostApiV2GlobalSystemMigrateError, GetTimeZonesAsyncData, GetTimeZonesAsyncResponse, GetTimeZonesAsyncError, CountTimezonesAsyncData, CountTimezonesAsyncResponse, CountTimezonesAsyncError, GetTimeZoneByIdAsyncData, GetTimeZoneByIdAsyncResponse, GetTimeZoneByIdAsyncError } from './types.gen';
import { client as _heyApiClient } from './client.gen';

export type Options<TData extends TDataShape = TDataShape, ThrowOnError extends boolean = boolean> = ClientOptions<TData, ThrowOnError> & {
    /**
     * You can provide a client instance returned by `createClient()` instead of
     * individual options. This might be also useful if you want to implement a
     * custom client.
     */
    client?: Client;
    /**
     * You can pass arbitrary values through the `meta` object. This can be
     * used to access values that aren't defined as part of the SDK function.
     */
    meta?: Record<string, unknown>;
};

export const getApiV2AiServiceCompletionsComplete = <ThrowOnError extends boolean = false>(options: Options<GetApiV2AiServiceCompletionsCompleteData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, GetApiV2AiServiceCompletionsCompleteError, ThrowOnError>({
        url: '/api/v2/AiService/Completions/Complete',
        ...options
    });
};

/**
 * Get all countries
 * Retrieves a list of all countries with optional OData pagination and filtering.
 */
export const getAllCountries = <ThrowOnError extends boolean = false>(options?: Options<GetAllCountriesData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetAllCountriesResponse, GetAllCountriesError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries',
        ...options
    });
};

/**
 * Count countries
 * Returns the total number of countries, with optional OData filtering.
 */
export const countCountries = <ThrowOnError extends boolean = false>(options?: Options<CountCountriesData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<CountCountriesResponse, CountCountriesError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/Count',
        ...options
    });
};

/**
 * Search countries by name
 * Searches for countries whose name matches the specified search term.
 */
export const searchCountriesByNameAsync = <ThrowOnError extends boolean = false>(options: Options<SearchCountriesByNameAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<SearchCountriesByNameAsyncResponse, SearchCountriesByNameAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/Search',
        ...options
    });
};

/**
 * Get country by ID
 * Retrieves a single country by its unique identifier.
 */
export const getCountryById = <ThrowOnError extends boolean = false>(options: Options<GetCountryByIdData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCountryByIdResponse, GetCountryByIdError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}',
        ...options
    });
};

/**
 * Get states for a country
 * Retrieves the list of states or provinces belonging to the specified country.
 */
export const getCountryStatesAsync = <ThrowOnError extends boolean = false>(options: Options<GetCountryStatesAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCountryStatesAsyncResponse, GetCountryStatesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/States',
        ...options
    });
};

/**
 * Count states for a country
 * Returns the total number of states or provinces for the specified country, with optional OData filtering.
 */
export const countCountryStatesAsync = <ThrowOnError extends boolean = false>(options: Options<CountCountryStatesAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<CountCountryStatesAsyncResponse, CountCountryStatesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/States/Count',
        ...options
    });
};

/**
 * Get state by ID
 * Retrieves a single state or province by its unique identifier within a country.
 */
export const getCountryStateByIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetCountryStateByIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCountryStateByIdAsyncResponse, GetCountryStateByIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/States/{countryStateId}',
        ...options
    });
};

/**
 * Get cities for a state
 * Retrieves the list of cities belonging to the specified state or province.
 */
export const getCitiesByCountryStateIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetCitiesByCountryStateIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCitiesByCountryStateIdAsyncResponse, GetCitiesByCountryStateIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/States/{countryStateId}/Cities',
        ...options
    });
};

/**
 * Count cities for a state
 * Returns the total number of cities for the specified state, with optional OData filtering.
 */
export const countCitiesByStateAsync = <ThrowOnError extends boolean = false>(options: Options<CountCitiesByStateAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<CountCitiesByStateAsyncResponse, CountCitiesByStateAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/States/{countryStateId}/Cities/Count',
        ...options
    });
};

/**
 * Get timezones for a country
 * Retrieves the list of timezones associated with the specified country.
 */
export const getTimeZonesByCountryIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetTimeZonesByCountryIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetTimeZonesByCountryIdAsyncResponse, GetTimeZonesByCountryIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/Timezones',
        ...options
    });
};

/**
 * Count timezones for a country
 * Returns the total number of timezones for the specified country, with optional OData filtering.
 */
export const countTimezonesByCountryAsync = <ThrowOnError extends boolean = false>(options: Options<CountTimezonesByCountryAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<CountTimezonesByCountryAsyncResponse, CountTimezonesByCountryAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/Timezones/Count',
        ...options
    });
};

/**
 * Get currencies for a country
 * Retrieves the list of enabled currencies for the specified country.
 */
export const getEnabledCurrenciesByCountryIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetEnabledCurrenciesByCountryIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetEnabledCurrenciesByCountryIdAsyncResponse, GetEnabledCurrenciesByCountryIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/Currencies',
        ...options
    });
};

/**
 * Get top-level domains for a country
 * Retrieves the list of internet top-level domains (TLDs) associated with the specified country.
 */
export const getTopLevelDomainsByCountryIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetTopLevelDomainsByCountryIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetTopLevelDomainsByCountryIdAsyncResponse, GetTopLevelDomainsByCountryIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/TopLevelDomains',
        ...options
    });
};

/**
 * Count top-level domains for a country
 * Returns the total number of top-level domains for the specified country, with optional OData filtering.
 */
export const countTopLevelDomainsByCountryAsync = <ThrowOnError extends boolean = false>(options: Options<CountTopLevelDomainsByCountryAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<CountTopLevelDomainsByCountryAsyncResponse, CountTopLevelDomainsByCountryAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/TopLevelDomains/Count',
        ...options
    });
};

/**
 * Get calling codes for a country
 * Retrieves the list of international telephone calling codes associated with the specified country.
 */
export const getCallingCodesByCountryIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetCallingCodesByCountryIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCallingCodesByCountryIdAsyncResponse, GetCallingCodesByCountryIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/CallingCodes',
        ...options
    });
};

/**
 * Count calling codes for a country
 * Returns the total number of calling codes for the specified country, with optional OData filtering.
 */
export const countCallingCodesByCountryAsync = <ThrowOnError extends boolean = false>(options: Options<CountCallingCodesByCountryAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<CountCallingCodesByCountryAsyncResponse, CountCallingCodesByCountryAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Countries/{countryId}/CallingCodes/Count',
        ...options
    });
};

/**
 * Get all currencies
 * Retrieves the list of all enabled currencies with optional OData pagination and filtering.
 */
export const getEnabledCurrenciesAsync = <ThrowOnError extends boolean = false>(options?: Options<GetEnabledCurrenciesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetEnabledCurrenciesAsyncResponse, GetEnabledCurrenciesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Currencies',
        ...options
    });
};

/**
 * Count currencies
 * Returns the total number of enabled currencies, with optional OData filtering.
 */
export const countCurrenciesAsync = <ThrowOnError extends boolean = false>(options?: Options<CountCurrenciesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<CountCurrenciesAsyncResponse, CountCurrenciesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Currencies/Count',
        ...options
    });
};

/**
 * Get currency by ID
 * Retrieves a single currency by its unique identifier.
 */
export const getCurrencyByIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetCurrencyByIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetCurrencyByIdAsyncResponse, GetCurrencyByIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Currencies/{currencyId}',
        ...options
    });
};

export const getVersion = <ThrowOnError extends boolean = false>(options?: Options<GetVersionData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        url: '/version',
        ...options
    });
};

export const getHealth = <ThrowOnError extends boolean = false>(options?: Options<GetHealthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        url: '/health',
        ...options
    });
};

export const getHello = <ThrowOnError extends boolean = false>(options?: Options<GetHelloData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        url: '/hello',
        ...options
    });
};

export const postRegister = <ThrowOnError extends boolean = false>(options: Options<PostRegisterData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, PostRegisterError, ThrowOnError>({
        url: '/register',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postLogin = <ThrowOnError extends boolean = false>(options: Options<PostLoginData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<PostLoginResponse, unknown, ThrowOnError>({
        url: '/login',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postRefresh = <ThrowOnError extends boolean = false>(options: Options<PostRefreshData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<PostRefreshResponse, unknown, ThrowOnError>({
        url: '/refresh',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const mapIdentityApiConfirmEmail = <ThrowOnError extends boolean = false>(options: Options<MapIdentityApiConfirmEmailData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        url: '/confirmEmail',
        ...options
    });
};

export const postResendConfirmationEmail = <ThrowOnError extends boolean = false>(options: Options<PostResendConfirmationEmailData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        url: '/resendConfirmationEmail',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postForgotPassword = <ThrowOnError extends boolean = false>(options: Options<PostForgotPasswordData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, PostForgotPasswordError, ThrowOnError>({
        url: '/forgotPassword',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postResetPassword = <ThrowOnError extends boolean = false>(options: Options<PostResetPasswordData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, PostResetPasswordError, ThrowOnError>({
        url: '/resetPassword',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postManage2Fa = <ThrowOnError extends boolean = false>(options: Options<PostManage2FaData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<PostManage2FaResponse, PostManage2FaError, ThrowOnError>({
        url: '/manage/2fa',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getManageInfo = <ThrowOnError extends boolean = false>(options?: Options<GetManageInfoData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetManageInfoResponse, GetManageInfoError, ThrowOnError>({
        url: '/manage/info',
        ...options
    });
};

export const postManageInfo = <ThrowOnError extends boolean = false>(options: Options<PostManageInfoData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<PostManageInfoResponse, PostManageInfoError, ThrowOnError>({
        url: '/manage/info',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postAccountPerformExternalLogin = <ThrowOnError extends boolean = false>(options?: Options<PostAccountPerformExternalLoginData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        ...formDataBodySerializer,
        url: '/Account/PerformExternalLogin',
        ...options,
        headers: {
            'Content-Type': null,
            ...options?.headers
        }
    });
};

export const postAccountLogout = <ThrowOnError extends boolean = false>(options?: Options<PostAccountLogoutData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        ...formDataBodySerializer,
        url: '/Account/Logout',
        ...options,
        headers: {
            'Content-Type': null,
            ...options?.headers
        }
    });
};

export const postAccountManageLinkExternalLogin = <ThrowOnError extends boolean = false>(options?: Options<PostAccountManageLinkExternalLoginData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        ...formDataBodySerializer,
        url: '/Account/Manage/LinkExternalLogin',
        ...options,
        headers: {
            'Content-Type': null,
            ...options?.headers
        }
    });
};

export const postAccountManageDownloadPersonalData = <ThrowOnError extends boolean = false>(options?: Options<PostAccountManageDownloadPersonalDataData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        url: '/Account/Manage/DownloadPersonalData',
        ...options
    });
};

/**
 * Get all languages
 * Retrieves the list of all supported languages with optional OData pagination and filtering.
 */
export const getLanguagesAsync = <ThrowOnError extends boolean = false>(options?: Options<GetLanguagesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetLanguagesAsyncResponse, GetLanguagesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Languages',
        ...options
    });
};

/**
 * Count languages
 * Returns the total number of supported languages, with optional OData filtering.
 */
export const countLanguagesAsync = <ThrowOnError extends boolean = false>(options?: Options<CountLanguagesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<CountLanguagesAsyncResponse, CountLanguagesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Languages/Count',
        ...options
    });
};

/**
 * Get language by ID
 * Retrieves a single language by its unique identifier.
 */
export const getLanguageByIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetLanguageByIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetLanguageByIdAsyncResponse, GetLanguageByIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Languages/{languageId}',
        ...options
    });
};

export const postApiV2GlobalSystemMigrate = <ThrowOnError extends boolean = false>(options?: Options<PostApiV2GlobalSystemMigrateData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostApiV2GlobalSystemMigrateResponse, PostApiV2GlobalSystemMigrateError, ThrowOnError>({
        url: '/api/v2/Global/System/Migrate',
        ...options
    });
};

/**
 * Get all timezones
 * Retrieves the list of all supported timezones with optional OData pagination and filtering.
 */
export const getTimeZonesAsync = <ThrowOnError extends boolean = false>(options?: Options<GetTimeZonesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetTimeZonesAsyncResponse, GetTimeZonesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Timezones',
        ...options
    });
};

/**
 * Count timezones
 * Returns the total number of supported timezones, with optional OData filtering.
 */
export const countTimezonesAsync = <ThrowOnError extends boolean = false>(options?: Options<CountTimezonesAsyncData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<CountTimezonesAsyncResponse, CountTimezonesAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Timezones/Count',
        ...options
    });
};

/**
 * Get timezone by ID
 * Retrieves a single timezone by its unique identifier.
 */
export const getTimeZoneByIdAsync = <ThrowOnError extends boolean = false>(options: Options<GetTimeZoneByIdAsyncData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetTimeZoneByIdAsyncResponse, GetTimeZoneByIdAsyncError, ThrowOnError>({
        url: '/api/v2/GlobeService/Timezones/{timeZoneId}',
        ...options
    });
};